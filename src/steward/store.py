...
class StewardStore:
    ...
    def append_receipt(self, run_id: str, payload: dict[str, Any], created_at: str) -> None:
        self.connection.execute(
            "INSERT INTO steward_receipts VALUES (?, ?, ?)",
            (run_id, json.dumps(payload, sort_keys=True), created_at),
        )
        self.connection.commit()

    def propose(self, proposal_id: str, payload: dict[str, Any]) -> None:
        self.connection.execute(
            "INSERT INTO steward_proposals VALUES (?, ?, 'PROPOSED_CHALLENGER')",
    ...
    def proposals(self) -> list[dict[str, Any]]:
        return [
            {"proposal_id": row[0], **json.loads(row[1]), "status": row[2]}
            for row in self.connection.execute(
                "SELECT proposal_id, payload, status FROM steward_proposals"
            )
        ]
