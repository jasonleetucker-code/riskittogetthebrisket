...
class SecretCredential:
    ...
    def __repr__(self) -> str:
        return f"SecretCredential(env_var={self.env_var!r}, value={_REDACTED!r})"
...
def redact(text: Any, secrets: Iterable[SecretCredential | str | None] = ()) -> str:
    ...
    out = _SECRET_PARAM.sub(lambda m: m.group(1) + _REDACTED, out)
    return _SECRET_HEADER.sub(lambda m: m.group(1) + _REDACTED, out)
