...
#: How many completed seasons of nflverse history the context is built
#: from.  Named because two other places have to agree with it — the
#: refresh owner that warms these caches
#: (``scripts/refresh_bdvm_inputs.py``) and the freshness report that
#: names the entries (``src/api/bdvm_api.py``).  A literal in each of the
#: three is three definitions of the window, and the two readers would
#: silently look at a cache entry the writer never fills.
DEFAULT_SEASONS_BACK = 6
