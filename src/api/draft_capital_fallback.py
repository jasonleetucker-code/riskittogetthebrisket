...
def _serialize_pick(
    ...
    row: dict[str, Any] = {
        ...
        "isExpansion": False,
        # ``None`` (not 0) when the contract could not price the pick, plus an
        # explicit flag so a consumer never has to infer "unpriced" from a
        # falsy dollar amount — a $0 rounding outcome on a real value is a
        # different state.  This is the D-2 / audit-finding-C1 fix; it must
        # survive any change to this serializer.
