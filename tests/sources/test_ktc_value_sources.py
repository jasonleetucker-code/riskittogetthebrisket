...
@pytest.mark.asyncio
async def test_selector_drives_visible_three_mode_control_not_a_guessed_parameter():
    page = _FakePage(
        [
            {
                "selected": 0,
    ...
    selected = await select_value_source(page, KTC_CROWD_TRADES)
    assert selected == {"label": "Crowd+Trades", "value": "both"}
    assert page.controls[1]["selected"] == 2


@pytest.mark.asyncio
async def test_selector_fails_closed_when_three_mode_control_disappears():
    page = _FakePage(
        [
            {
                "selected": 0,
    ...
    with pytest.raises(KtcValueSourceError, match="three-mode Value Source control not found"):
        await select_value_source(page, KTC_CROWD_TRADES)
