...
single-worst-player fallback, still relied on by Playoff MVP — see
``src/public_league/awards.py::_playoff_mvp_player_rows``, whose
population is structurally small (one team's own playoff starters)
independent of season progress, so a full-band requirement would exclude
it always rather than only early on.
"""
