...
def _row(name: str, pos: str, *, ktc=None, idp=None, extra=None) -> dict:
    ...
    sites: dict = {}
    if ktc is not None:
        # One KTC model input (Crowd) plus KTC's own published market value
        # (Crowd+Trades, benchmark-only) so ``ktcRank`` — KTC Market's rank —
        # is defined on these fixtures.
...
class TestDNoOffenseRegression(unittest.TestCase):
    ...
    def test_offense_ranks_are_untouched(self):
        ...
        for r in rows:
            meta = r["sourceRankMeta"]["ktcCrowdSfTep"]
            self.assertEqual(meta["method"], TRANSLATION_DIRECT)
    ...
    def test_ktc_does_not_rank_idp_players(self):
        ...
        self.assertNotIn("ktcCrowdSfTep", dl["sourceRanks"])
...
class TestETransparencyFields(unittest.TestCase):
    ...
    def test_legacy_rank_fields_mirror_source_ranks(self):
        ...
        # ``ktcRank`` is KTC MARKET's own rank (benchmark), not a vote's rank.
        self.assertEqual(qb["ktcRank"], qb["ktcMarket"]["rank"])
...
class TestGDualScopeIdpTradeCalc(unittest.TestCase):
    ...
    def test_offense_players_get_ktc_and_idptc_ranks(self):
        ...
        for r in (qb1, wr1, rb1):
            self.assertIn("ktcCrowdSfTep", r["sourceRanks"])
        ...
        self.assertEqual(qb1["sourceRanks"]["ktcCrowdSfTep"], 1)
        self.assertEqual(wr1["sourceRanks"]["ktcCrowdSfTep"], 2)
        self.assertEqual(rb1["sourceRanks"]["ktcCrowdSfTep"], 3)
    ...
    def test_offense_idptc_rank_fed_into_blend(self):
        ...
        self.assertEqual(wr1["sourceRanks"]["ktcCrowdSfTep"], 1)
        self.assertEqual(wr1["sourceRanks"]["idpTradeCalc"], 2)
        self.assertEqual(wr2["sourceRanks"]["ktcCrowdSfTep"], 2)
    ...
    def test_idp_players_unaffected_by_extra_offense_scope(self):
        ...
        for r in rows:
            ...
            self.assertNotIn("ktcCrowdSfTep", r["sourceRanks"])
    ...
    def test_offense_player_without_idptc_value_still_ranks_via_ktc(self):
        ...
        still land on the unified board.

        With two primary-scope offense sources registered (KTC + DLF SF),
        a KTC-only offense player IS a semantic single-source — both KTC
        and DLF SF are structurally expected for offense players and DLF
        did not match.  The guarantee this test protects is that the
        player still RANKS (lands on the board); the 1-src flagging is
        correct behavior.
        """
        rows = [
            _row("qb_solo", "QB", ktc=8000),  # no IDPTC / DLF SF value
        ...
        self.assertEqual(solo["sourceRanks"], {"ktcCrowdSfTep": 2})
        ...
        self.assertEqual(both["sourceRanks"]["ktcCrowdSfTep"], 1)
...
class TestHCrossUniverseRanking(unittest.TestCase):
    ...
    def test_single_scope_source_unchanged_by_combined_pass(self):
        ...
        _compute_unified_rankings(rows, {})
        self.assertEqual(rows[0]["sourceRanks"]["ktcCrowdSfTep"], 1)
        self.assertEqual(rows[1]["sourceRanks"]["ktcCrowdSfTep"], 2)
        self.assertEqual(rows[2]["sourceRanks"]["ktcCrowdSfTep"], 3)
        dl1 = next(r for r in rows if r["canonicalName"] == "dl1")
        self.assertNotIn("ktcCrowdSfTep", dl1["sourceRanks"])
