...
class TestEmptyAndAbsentSources:
    ...
    def test_a_source_missing_from_every_row_does_not_disturb_the_others(self):
        ...
        """
        with_absent = [
            _row("Star", "WR", ktcCrowdSfTep=9000, idpTradeCalc=9000),
            _row("Mid", "WR", ktcCrowdSfTep=4500, idpTradeCalc=4500),
            _anchor_qb(),
        ]
...
class TestZeroVariance:
    ...
    def test_identical_values_across_pool_do_not_divide_by_zero(self):
        rows = [_row(f"Clone {i}", "WR", ktcCrowdSfTep=5000, idpTradeCalc=5000) for i in range(5)]
        dc._compute_unified_rankings(rows, {})
        for r in rows:
    ...
    def test_identical_values_within_one_player_give_zero_spread(self):
        rows = [
            _row("Agreed", "WR", ktcCrowdSfTep=5000, idpTradeCalc=5000),
            _anchor_qb(),
        ]
        dc._compute_unified_rankings(rows, {})
...
class TestNonPositiveAndNonFiniteValues:
    def test_negative_zero_and_none_are_all_treated_as_absent(self):
        ...
        """
        rows = [
            _row("Neg", "WR", ktcCrowdSfTep=-500, idpTradeCalc=3000),
            _row("Zero", "WR", ktcCrowdSfTep=0, idpTradeCalc=3000),
            _row("Nil", "WR", ktcCrowdSfTep=None, idpTradeCalc=3000),
            _anchor_qb(),
        ]
        dc._compute_unified_rankings(rows, {})
        got = _by_name(rows)
...
def _payload(corrupt: Any = None) -> dict[str, Any]:
    ...
    for i in range(n):
        ...
        players[f"Player {i:03d}"] = {
            ...
            "team": "FA",
            "_sites": 2,
            "_canonicalSiteValues": {"ktcCrowdSfTep": ktc, "idpTradeCalc": ktc},
        }
    if corrupt is not None:
        players["Glitch Guy"] = {
            ...
            "team": "FA",
            "_sites": 2,
            "_canonicalSiteValues": {"ktcCrowdSfTep": corrupt, "idpTradeCalc": 4000},
        }
...
class TestTiesAndDuplicates:
    def test_tied_source_values_produce_identical_player_values(self):
        ...
        """
        rows = [
            _row("Tie A", "WR", ktcCrowdSfTep=4000, idpTradeCalc=4000),
            _row("Tie B", "WR", ktcCrowdSfTep=4000, idpTradeCalc=4000),
            _row("Lower", "WR", ktcCrowdSfTep=3000, idpTradeCalc=3000),
            _anchor_qb(),
        ]
        dc._compute_unified_rankings(rows, {})
        got = _by_name(rows)
    ...
    def test_duplicate_names_are_valued_independently(self):
        ...
        """
        rows = [
            _row("Josh Allen", "QB", ktcCrowdSfTep=9000, idpTradeCalc=9000),
            _row("Josh Allen", "LB", ktcCrowdSfTep=2000, idpTradeCalc=2000),
            _anchor_qb(),
        ]
        dc._compute_unified_rankings(rows, {})
...
class TestNoNaNEscapes:
    def test_no_stamped_value_is_nan(self):
        ...
        rows = [
            _row("Split", "WR", ktcCrowdSfTep=9000, idpTradeCalc=1000),
            _row("Solo", "WR", ktcCrowdSfTep=4000),
