...
``ktcCrowdSfTep=99990`` therefore deflated EVERY other player by ~45%, with
...
class TestDeclaredRangeCheck:
    def test_in_range_values_accepted(self):
        assert _value_is_in_declared_range("ktcCrowdSfTep", 0.0)
        assert _value_is_in_declared_range("ktcCrowdSfTep", 5000.0)
        assert _value_is_in_declared_range("ktcCrowdSfTep", 9999.0)

    def test_out_of_range_values_rejected(self):
        assert not _value_is_in_declared_range("ktcCrowdSfTep", 10000.0)
        assert not _value_is_in_declared_range("ktcCrowdSfTep", 99990.0)
        assert not _value_is_in_declared_range("ktcCrowdSfTep", -1.0)
...
class TestPolicyB_SingleBadRowDropped:
    def test_site_max_ignores_the_out_of_range_row(self):
        ...
        rows = _rows("ktcCrowdSfTep", [9999.0] + [5000.0] * 99 + [99990.0])
        value_source_max, suppressed, diags = _partition_value_source_ranges(rows)

        assert value_source_max["ktcCrowdSfTep"] == 9999.0, (
            "site_max must exclude the out-of-range row; "
            "pre-fix this was 99990 and deflated every player ~45%"
        )
        assert "ktcCrowdSfTep" not in suppressed
        assert diags["ktcCrowdSfTep"] == {"total": 101, "outOfRange": 1}
    ...
    def test_one_bad_row_does_not_suppress_the_source(self):
        ...
        rows = _rows("ktcCrowdSfTep", [9999.0] + [5000.0] * 199 + [99990.0])
        _, suppressed, diags = _partition_value_source_ranges(rows)
        assert diags["ktcCrowdSfTep"]["outOfRange"] == 1
        assert diags["ktcCrowdSfTep"]["total"] == 201
        assert "ktcCrowdSfTep" not in suppressed
...
class TestPolicyC_ScaleChangeSuppresses:
    def test_many_out_of_range_rows_suppress_the_source(self):
        ...
        rows = _rows("ktcCrowdSfTep", [50000.0] * 30 + [5000.0] * 70)
        _, suppressed, diags = _partition_value_source_ranges(rows)
        assert diags["ktcCrowdSfTep"]["outOfRange"] == 30
        assert "ktcCrowdSfTep" in suppressed
    ...
    def test_threshold_boundary_does_not_suppress(self):
        ...
        rows = _rows("ktcCrowdSfTep", [50000.0] * bad + [5000.0] * (total - bad))
...
class TestCleanBoardUnaffected:
    def test_healthy_board_produces_no_drops_and_no_suppression(self):
        ...
        rows = _rows("ktcCrowdSfTep", [9999.0, 7000.0, 4000.0, 1.0, 0.0])
        value_source_max, suppressed, diags = _partition_value_source_ranges(rows)
        assert value_source_max["ktcCrowdSfTep"] == 9999.0
        assert suppressed == set()
        assert diags["ktcCrowdSfTep"]["outOfRange"] == 0

    def test_non_numeric_and_missing_values_are_skipped_not_counted(self):
        rows = [
            {"canonicalSiteValues": {"ktcCrowdSfTep": 9999.0}},
            {"canonicalSiteValues": {"ktcCrowdSfTep": None}},
            {"canonicalSiteValues": {"ktcCrowdSfTep": "not-a-number"}},
            {"canonicalSiteValues": None},
            {},
        ]
        value_source_max, suppressed, diags = _partition_value_source_ranges(rows)
        assert value_source_max["ktcCrowdSfTep"] == 9999.0
        assert diags["ktcCrowdSfTep"]["total"] == 1
        assert suppressed == set()
...
class TestEndToEndDeflation:
    ...
    def test_top_asset_keeps_full_value_despite_a_corrupt_row(self, bad_value):
        rows = _rows("ktcCrowdSfTep", [9999.0] + [5000.0] * 99 + [bad_value])
        value_source_max, suppressed, _ = _partition_value_source_ranges(rows)
        site_max = value_source_max["ktcCrowdSfTep"]
...
class TestEscalationNeedsAdequateSample:
    ...
    guard suppressed ktcCrowdSfTep on a 4-row fixture holding a single bad
    value.
    """
    ...
    def test_tiny_sample_never_escalates_even_at_high_fraction(self):
        ...
        rows = _rows("ktcCrowdSfTep", [9999.0, 8000.0, 5000.0, 99990.0])
        assert len(rows) < _VALUE_RANGE_ESCALATION_MIN_ROWS
        value_source_max, suppressed, diags = _partition_value_source_ranges(rows)

        assert diags["ktcCrowdSfTep"]["outOfRange"] == 1
        assert diags["ktcCrowdSfTep"]["total"] == 4  # 25%, far above the 2% fraction
        assert (
            "ktcCrowdSfTep" not in suppressed
    ...
    def test_adequate_sample_still_escalates(self):
        ...
        bad = int(n * 0.5)
        rows = _rows("ktcCrowdSfTep", [50000.0] * bad + [5000.0] * (n - bad))
        _, suppressed, _ = _partition_value_source_ranges(rows)
        assert "ktcCrowdSfTep" in suppressed
