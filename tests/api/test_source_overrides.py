...
class TestCustomMixIsDisabled(unittest.TestCase):
    ...
    def test_a_posted_source_mix_is_ignored(self) -> None:
        out, warnings = normalize_source_overrides(
            {"ktcCrowdSfTep": {"include": False}, "dlfSf": {"weight": 3.0}}
        )
    ...
    def test_it_is_answered_rather_than_refused(self) -> None:
        ...
        out, warnings = normalize_source_overrides({"ktcCrowdSfTep": {"include": False}})
...
class TestNormalizeSourceOverrides(unittest.TestCase):
    ...
    def test_legacy_site_weights_shape(self) -> None:
        out, warnings = normalize_source_overrides(
            {"ktcCrowdSfTep": {"include": False}, "dlfSf": {"weight": 0.5}}
        )
        self.assertEqual(out["ktcCrowdSfTep"], {"include": False})
    ...
    def test_invalid_weight_is_rejected(self) -> None:
        out, warnings = normalize_source_overrides({"ktcCrowdSfTep": {"weight": "not a number"}})
        self.assertNotIn("weight", out.get("ktcCrowdSfTep", {}))
        self.assertTrue(warnings)
        out, warnings = normalize_source_overrides({"ktcCrowdSfTep": {"weight": -1}})
        self.assertNotIn("weight", out.get("ktcCrowdSfTep", {}))
        self.assertTrue(warnings)
        out, warnings = normalize_source_overrides({"ktcCrowdSfTep": {"weight": float("inf")}})
        self.assertNotIn("weight", out.get("ktcCrowdSfTep", {}))
        self.assertTrue(warnings)

    def test_include_non_bool_is_rejected(self) -> None:
        out, warnings = normalize_source_overrides({"ktcCrowdSfTep": {"include": "yes"}})
        self.assertNotIn("include", out.get("ktcCrowdSfTep", {}))
        self.assertTrue(warnings)
...
class TestBuildApiDataContractDefaultPath(unittest.TestCase):
    ...
    def test_default_payload_has_rankings(self) -> None:
        ...
        self.assertIn("ktcCrowdSfTep", allen.get("sourceRanks", {}))
...
class TestBuildApiDataContractOverridePath(unittest.TestCase):
    ...
    def test_disabling_source_removes_it_from_every_stamp(self) -> None:
        overridden = build_api_data_contract(
            ...
            source_overrides={"ktcCrowdSfTep": {"include": False}},
        )
        ...
        for row in overridden.get("playersArray", []):
            ...
            self.assertNotIn("ktcCrowdSfTep", ranks)

        # Josh Allen loses one signal but stays on the board.
        allen = by_name.get("Josh Allen")
        self.assertIsNotNone(allen)
        self.assertNotIn("ktcCrowdSfTep", allen.get("sourceRanks", {}))
        self.assertIsNotNone(allen.get("canonicalConsensusRank"))
    ...
    def test_override_rankings_override_block_reflects_config(self) -> None:
        override = {"ktcCrowdSfTep": {"include": False}, "dlfSf": {"weight": 0.5}}
        ...
        self.assertNotIn("ktcCrowdSfTep", rov.get("enabledSources") or [])
...
class TestRankingsTradeCalculatorAlignment(unittest.TestCase):
    ...
    def test_override_response_stamps_consistent_fields(self) -> None:
        override = {"dlfSf": {"include": False}, "ktcCrowdSfTep": {"weight": 2.0}}
        contract = build_api_data_contract(_fixture_raw_payload(), source_overrides=override)
        for row in contract.get("playersArray", []):
...
class TestSummarizeSourceOverrides(unittest.TestCase):
    ...
    def test_explicit_default_weight_is_not_customized(self) -> None:
        summary = _summarize_source_overrides({"ktcCrowdSfTep": {"weight": 1.0}})
        self.assertFalse(summary["isCustomized"])

    def test_excluded_source_marks_customized(self) -> None:
        summary = _summarize_source_overrides({"ktcCrowdSfTep": {"include": False}})
        self.assertTrue(summary["isCustomized"])
        self.assertNotIn("ktcCrowdSfTep", summary["enabledSources"])

    def test_non_default_weight_marks_customized(self) -> None:
        summary = _summarize_source_overrides({"ktcCrowdSfTep": {"weight": 2.0}})
        self.assertTrue(summary["isCustomized"])
        self.assertEqual(summary["weights"]["ktcCrowdSfTep"], 2.0)
...
class TestBuildRankingsDeltaPayload(unittest.TestCase):
    ...
    def test_delta_byte_size_is_bounded(self) -> None:
        ...
        #
        # Bumped 65KB → 70KB 2026-09-23 (owner directive: KTC Crowd and KTC
        # Trades are two separate model inputs).  A second KTC voter means
        # every KTC-covered row carries one more sourceRanks/sourceRankMeta/
        # effectiveSourceRanks entry, plus the compact canonical ``ktcMarket``
        # benchmark block and the per-row source-weight state.  Freshness
        # stamps were trimmed to reduced observations only before measuring:
        # 66,459 bytes on this fixture.
        self.assertLess(delta_bytes, 70_000)
...
class TestNormalizeTepMultiplier(unittest.TestCase):
    ...
    def test_missing_field_returns_none(self) -> None:
        ...
        self.assertIsNone(normalize_tep_multiplier({"ktcCrowdSfTep": {"include": False}}))
    ...
    def test_tep_multiplier_alongside_legacy_overrides(self) -> None:
        ...
        """
        body = {"tep_multiplier": 1.2, "ktcCrowdSfTep": {"include": False}}
        overrides, warnings = normalize_source_overrides(body)
        self.assertEqual(overrides, {"ktcCrowdSfTep": {"include": False}})
        self.assertEqual(warnings, [])
...
class TestNormalizeTepNativeMultiplier(unittest.TestCase):
    ...
    def test_missing_field_returns_none(self) -> None:
        ...
        self.assertIsNone(normalize_tep_native_multiplier({"ktcCrowdSfTep": {"include": False}}))
...
class TestTepMultipliersEndToEnd(unittest.TestCase):
    ...
    def test_ktc_exemption_holds_under_default_and_override(self) -> None:
        ...
        ``ktcCrowdSfTep``) stay exempt from BOTH TEP correction paths, with
        ...
        # ktcCrowdSfTep from the exemption fails this test before the
        # behavioural assertions below would.
        self.assertIn("ktcCrowdSfTep", _TE_BLANKET_KTC_EXEMPT_KEYS)
        ...
        for label, kwargs in (
            ...
            ktc_meta = (bowers.get("sourceRankMeta") or {}).get("ktcCrowdSfTep") or {}
            # The exempt source carries no boost / correction flag, so
            # ``valueContribution`` is the raw curve value untouched.
            self.assertNotIn(
                "tepBoostApplied",
                ktc_meta,
                f"{label}: ktcCrowdSfTep must not be marked TEP-boosted on a TE row",
            )
            self.assertNotIn(
                "tepNativeCorrectionApplied",
                ktc_meta,
                f"{label}: ktcCrowdSfTep must not be marked TEP-native-corrected on a TE row",
            )
            self.assertNotIn(
                "tepMultiplier",
                ktc_meta,
                f"{label}: ktcCrowdSfTep meta must not stamp a tepMultiplier value",
            )
            self.assertNotIn(
                "tepNativeCorrection",
                ktc_meta,
                f"{label}: ktcCrowdSfTep meta must not stamp a tepNativeCorrection value",
            )
...
class TestTheLeagueLensIsWithdrawnFromThisEndpoint(unittest.TestCase):
    ...
    def test_the_override_delta_still_serves_a_ranked_board(self) -> None:
        ...
        payload = build_rankings_delta_payload(
            _fixture_raw_payload(), source_overrides={"ktcCrowdSfTep": {"include": False}}
        )
