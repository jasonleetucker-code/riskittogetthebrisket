...
in ``data_contract`` speaks the ranking-registry KEY (``ktcCrowdSfTep``,
...
class TestRunNameResolvesToRegistryKeys:
    def test_ktc_run_name_round_trips_to_its_registry_key(self):
        # One KTC scraper run produces BOTH KTC model inputs (owner directive
        # 2026-09-23); a KTC run failure governs both registry keys.
        assert registry_keys_for_run_source("KTC") == ["ktcCrowdSfTep", "ktcTradesSfTep"]
