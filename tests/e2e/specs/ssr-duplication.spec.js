 ...
 * how streamed content arrives. Asserting its mere presence would fail
 * constantly on a perfectly healthy app.
 *
 ...
 * no longer a defect.
 *
 * PROVENANCE (V1-112, 2026-08-21): ported from the still-open, never-merged
 * PR #758 (`tests/e2e/specs/ssr-duplication.spec.js`, 2026-08-05). Ported
 * verbatim — the invariant, the helpers it depends on (`awaitStreamSettled`,
 * `pageUrl`), and the three target routes all still exist unchanged on
 * current main. NOT ported from that PR: `frontend/scripts/measure-duplication.mjs`
 * (the 2,400-load rate-measurement harness — this sprint's L2 evidence is a
 * live run of this deterministic spec, not a fresh statistical study) and the
 * unrelated `journey-tools-health.spec.js` polling fix (a different flake,
 * out of scope for this row).
 */
const { test, expect } = require("../helpers/auth-fixture");
const { pageUrl, awaitStreamSettled } = require("../helpers/journey");

// Private routes, chosen as the heaviest streamers measured in #758: /rankings
// (46% of loads staged mid-flight) and /waivers + /arbitrage (the two the
