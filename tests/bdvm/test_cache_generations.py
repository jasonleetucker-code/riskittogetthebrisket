"""BDVM auxiliary caches must follow the evidence and the scoring actually consumed.

Harvested from #1346 (donor commit 7c9d5b2e7, ``tests/bdvm/test_cache_generations.py``),
adapted to main: the donor's single-flight concurrency test is not carried
(single-flight is part of the excluded prepared-serving campaign); the
cache-key, bounding and failure-retry properties are.
"""
...
def test_actuals_reload_after_same_day_input_refresh():
    ...
    with mock.patch.object(actuals, "fetch_current_season_actuals", side_effect=scored):
        ...
        cache.put(key, [{"points": 123, "pad": "x" * 10}])
        assert api._actuals_for(_contract())[1]["receiver"] == [(1, 123)]


def test_actuals_reload_after_pbp_supplement_refresh():
    path = pbp_weekly.pbp_weekly_path(2026)
    path.parent.mkdir(parents=True)
    path.write_text("generation one")
    with mock.patch.object(actuals, "fetch_current_season_actuals", return_value=(2, {})) as run:
        api._actuals_for(_contract())
        api._actuals_for(_contract())
        path.write_text("generation two, changed")
        api._actuals_for(_contract())
    assert run.call_count == 2


def test_failed_actuals_read_is_not_memoized():
    with mock.patch.object(
        actuals, "fetch_current_season_actuals", side_effect=[OSError("busy"), (2, {})]
    ) as run:
        assert api._actuals_for(_contract()) == (None, {})
        assert api._actuals_for(_contract()) == (2, {})
        assert api._actuals_for(_contract()) == (2, {})
    assert run.call_count == 2
...
def test_auxiliary_caches_are_bounded():
    ...
    assert len(api._actuals_cache) <= api._AUX_CACHE_MAX
    with mock.patch.object(context_store, "load_snapshot", return_value=None):
        for season in range(2000, 2040):
            api._context_for(season)
    assert len(api._context_cache) <= api._AUX_CACHE_MAX
    with mock.patch.object(schedule, "fetch_team_weeks", return_value=None):
        for season in range(2000, 2040):
            api._schedule_for(season)
    assert len(api._schedule_cache) <= api._AUX_CACHE_MAX
...
def test_failed_actuals_read_is_retried_and_report_uses_season_not_week(tmp_path):
    ...
    with (
        ...
        assert load.call_count == 2
        assert run.call_count == 2
        # The report is keyed by the actuals SEASON, never the observed week (2).
