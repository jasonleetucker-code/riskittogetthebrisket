...
base.  git, the reconciler and the install path are all real; only
``sudo``/``systemctl`` and the parts of a rollback that would touch a
machine (python venv, frontend build, service restart, verify-deploy)
are stubbed.
"""
...
case "${cmd}" in
  ...
  enable)
    ...
    [[ -f "${SYSTEMD_UNIT_DIR}/${unit}" ]] || {
      printf 'Failed to enable unit: %s does not exist.\\n' "${unit}" >&2; exit 1; }
    ...
    [[ -f "${SYSTEMD_UNIT_DIR}/${svc}" ]] && printf 'loaded\\n' > "${state}/units/${svc}.LoadState"
    exit 0
    ;;
esac
exit 0
...
class RollbackWorld:
    ...
    def install_newer_runtime_controls(self):
        ...
        shutil.copy2(SYSD / "dynasty-healthcheck.sh", self.lib_dir / "dynasty-healthcheck.sh")
    ...
    def rollback(self, extra_env: dict[str, str] | None = None):
        env = {
            **os.environ,
            ...
            # Test seams: rollback.sh scrubs these before sourcing the
            # reconciler, so the fixture re-exports them through the
            # documented test-mode gate instead.
            "RC_ALLOW_TEST_OVERRIDES": "1",
            "SYSTEMD_UNIT_DIR": str(self.unit_dir),
            "RC_PROC_DIR": str(self.root / "proc"),
...
class TestTheFirstRollbackPastTheReconciler:
    ...
    def test_runtime_controls_are_reconciled_to_the_target(self, world):
        world.build()
        world.install_newer_runtime_controls()
        newer_unit = (world.unit_dir / f"{SERVICE_NAME}.service").read_bytes()
        ...
        assert (
            world.unit_dir / f"{SERVICE_NAME}.service"
        ).read_bytes() != newer_unit, "the newer deploy's unit survived the rollback"
        assert "runtime controls reconciled and verified" in r.stdout
    ...
    def test_the_preserved_copy_lives_outside_the_checkout(self, world):
        """Sourcing it from a path `git checkout` rewrites would mean the
        file could change under the step meant to survive the change."""
        world.build()
        world.install_newer_runtime_controls()

        r = world.rollback()
        assert r.returncode == 0, r.stdout + r.stderr
        # Nothing left behind, and never read from APP_DIR.
    ...
    def test_a_target_without_runtime_artifacts_fails_loudly(self, world):
        ...
        being rolled back FROM is the failure being fixed, so this stops
