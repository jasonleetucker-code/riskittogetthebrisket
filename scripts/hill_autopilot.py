...
def _forward_scores(champ, winner: CandidateScore) -> tuple[list[ForwardScore], list[dict[str, Any]]]:
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: current tournament could not be evaluated: {exc}", file=sys.stderr)
        return 2
