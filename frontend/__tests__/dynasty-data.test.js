...
// only importer was this file.  It claimed to mirror
// `src/utils/name_clean.py::normalize_player_name` and did not (no
// apostrophe rule), and the cases below passed anyway because none of
// them used an apostrophe — a test suite validating a duplicate
// against itself.
//
// The verified mirror is `lib/player-name-match.js::normalizePlayerNameKey`.
// These cases moved to `__tests__/player-name-match.test.js`, which now
// also asserts cross-language parity against the shared fixture
// `tests/fixtures/name_key_cases.json` (see
