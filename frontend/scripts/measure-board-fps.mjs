...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (argv[i] === "--ablate") {
      // Raw CSS injected before measuring, so a suspect can be removed
      // from the LIVE board without editing a stylesheet and rebuilding.
      // This is how the row-count question was settled and how the
      // residual after windowing was attributed; a rebuild between arms
      // would let something else change too.
      out.ablate = argv[++i];
      out.ablateLabel = out.ablate.replace(/\s+/g, " ").slice(0, 60);
    }
  }
  return out;
}
...
// windowed — that is the whole point of windowing. `aria-rowcount`
// carries the full row count when a window is active; without a window
...
// fires.
//
// It reads the attribute off the <table>, not the <tbody>. This script
// looked at <tbody> and found nothing, so it fell through to the mounted
// count and the guard below fired on the windowed run — the same
// failure the paragraph above describes, one element up. <table> is
// where ARIA puts it: `aria-rowcount` belongs on the `table`/`grid`
// role, and a <tbody> is a `rowgroup`.
const boardCount = await page.evaluate(() => {
  const table = document.querySelector(".ds-table-wrap table");
  // -1 for the header row, which aria-rowcount includes.
  const declared = Number(table?.getAttribute("aria-rowcount") || 0);
  return declared > 0
    ? declared - 1
...
// The >=50-row E2E assertions must stay true, so check the same thing
// here rather than finding out from a red suite later — but check what
// they actually assert. They ask how many rows the board HOLDS
// (journey.js::boardRowCount reads the same aria-rowcount), because a
// windowed board mounts far fewer than 50 by design and a mounted-count
// guard would refuse every successful run. This one refused a run at 38
// mounted rows, which was windowing working.
if (boardCount < 50) {
  console.error(
    `REFUSING TO REPORT: the board holds only ${boardCount} rows. ` +
...
if (args.ablate) {
  await page.addStyleTag({ content: args.ablate });
  // Let the removal settle before sampling: an ablation that changes
  // layout would otherwise have its own reflow counted as scroll cost.
