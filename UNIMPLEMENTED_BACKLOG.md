...
| **branch protection on `main`** | **Was missing from this register** — it lived only in `docs/OWNER_ACTION_AUDIT_2026-07-29.md` as OA-02, so the two lists disagreed about what was outstanding. `main` is unprotected (`"protected": false`) while `deploy.yml` ships every push to it straight to production. Payload + apply script are committed and inert: `GH_TOKEN=<admin> bash deploy/github/apply-branch-protection.sh` (`--dry-run` first). Needs an **admin-scoped** token, which no session credential has. |
| **`INTEL_REFRESH_TOKEN` rotation** | Unverified whether completed. Runbook: `docs/runbooks/rotate-intel-refresh-token.md`. **Start at its Step 1** — `server.py` falls back to `SIGNAL_ALERT_CRON_TOKEN` and `.env.example` ships the key commented out, so prod may be authenticating on the fallback, which makes this an *add* rather than a rotate. Which one is true cannot be determined from the repo. |
| **certbot renewal dry-run** | `sudo bash deploy/verify-cert-renewal.sh` on the VPS — checks the timer is armed, that `renew_hook` reloads nginx (certs use the **webroot** plugin, so certbot never touches nginx and without the hook a renewed cert sits unused), and runs `certbot renew --dry-run` against the *current* config. Cannot be run from a session: no ssh client, and the egress proxy re-signs TLS so any cert read from here is the proxy's (`ORCHESTRATION.md` §6.5). |
