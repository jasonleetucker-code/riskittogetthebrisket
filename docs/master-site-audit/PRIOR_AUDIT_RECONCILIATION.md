...
| `PRIOR-A21-F01` (High) — public trade grades apply linear thresholds to an α=1.65-transformed gap | W19-F009 | The cited code no longer exists — a `grep -rn` for `_GRADE_ALPHA` or `1.65` across `src/public_league/` returns nothing. Grading moved to `trade_grading.py`; served grades recomputed independently and matched byte for byte |
