...
def count_aware(values):
    ...
    sv=sorted(values); k=len(sv)
    if k==1: return sv[0], None
    if k==2: return (sv[0]+sv[1])/2.0, abs(sv[0]-sv[1])/2.0
...
def hampel(pairs,k=HAMPEL_K,min_n=HAMPEL_MIN_N,min_threshold=HAMPEL_MIN_THRESHOLD):
    ...
    if len(kept)<2: return list(pairs), []
    return kept, dropped
...
for r in pa:
    ...
    hampel_stats['rows']+=1
    if dropped: hampel_stats['rows_with_drop']+=1
    ...
    if use_hier:
        if anchor is not None and subc is not None:
            center = anchor + ALPHA*(subc-anchor); delta=subc-anchor
        elif anchor is not None: center=anchor; delta=None
        elif subc is not None: center=subc; delta=None
        else: center=0.0; delta=None
    else:
    ...
    if (not is_pick) and len(all_v)<=1:
        blended*=SINGLE_SOURCE_RETENTION; haircut=True
        single_source_rows.append((r.get('canonicalName'),posn,r.get('rankDerivedValue'),blended,all_v))
    results.append(dict(name=r.get('canonicalName'),pos=posn,is_pick=is_pick,
                        pred=blended, uncapped=r.get('_blendedValueUncapped'),
                        rdv=r.get('rankDerivedValue'),
                        anchor_pred=anchor, anchor_stamp=r.get('anchorValue'),
                        sub_pred=subc, sub_stamp=r.get('subgroupBlendValue'),
                        haircut=haircut, dropped=dropped,
                        dropped_stamp=r.get('droppedSources'),
                        rank=r.get('canonicalConsensusRank'),
                        alpha=r.get('alphaShrinkage')))

json.dump(results, open('/tmp/claude-0/-home-user-riskittogetthebrisket/0f0078ff-84f2-50d3-bce6-2bb1d1d8e920/scratchpad/repro.json','w'))
print('rows reproduced', len(results))
print('HAMPEL: rows', hampel_stats['rows'], 'rows_with_drop', hampel_stats['rows_with_drop'],
      'pair drop rate', round(hampel_stats['dropped_pairs']/max(1,hampel_stats['pairs'])*100,2),'%',
      'row drop rate', round(hampel_stats['rows_with_drop']/max(1,hampel_stats['rows'])*100,2),'%')
print()
print('per-source drop rates:')
for sk in sorted(per_source_total, key=lambda s:-per_source_dropped[s]/max(1,per_source_total[s])):
    t=per_source_total[sk]; d=per_source_dropped[sk]
