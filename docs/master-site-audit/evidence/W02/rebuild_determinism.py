...
SCRATCH = Path('/tmp/claude-0/-home-user-riskittogetthebrisket/0f0078ff-84f2-50d3-bce6-2bb1d1d8e920/scratchpad')
...
if ha != hb:
    ...
    for n, ra in pa.items():
        rb = pb.get(n)
        if rb is None:
            diffs.append((n, 'missing', None, None)); continue
        for k in ra:
            if json.dumps(ra[k], sort_keys=True, default=str) != json.dumps(rb.get(k), sort_keys=True, default=str):
