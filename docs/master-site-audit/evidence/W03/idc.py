...
for r in pa:
    ...
    if got is None or abs(exp-got)>1e-9: bad.append((r['displayName'],r.get('assetClass'),r.get('playerId'),exp,got))
print('mismatch',len(bad)); print(bad[:8])
...
q=collections.Counter(r.get('quarantined') for r in pa); print('quarantined',q.most_common())
af=collections.Counter()
for r in pa:
    for f in (r.get('anomalyFlags') or []): af[f]+=1
