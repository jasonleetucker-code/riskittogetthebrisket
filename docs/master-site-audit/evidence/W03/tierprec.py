...
ex = []
for b in bounds:
    a = byname.get(b.player_above)
    c = byname.get(b.player_below)
    if not a or not c:
        continue
    sa = a.get("sourceSpread")
    sc = c.get("sourceSpread")
    ...
    if b.raw_gap < m:
        worse += 1
        if len(ex) < 6:
            ex.append((b.player_above, b.player_below, b.raw_gap, round(sa, 1), round(sc, 1)))
print(
...
print(
    "sourceSpread over ranked rows: median",
    round(statistics.median(ss), 1),
    "p90",
    round(sorted(ss)[int(0.9 * len(ss))], 1),
)
