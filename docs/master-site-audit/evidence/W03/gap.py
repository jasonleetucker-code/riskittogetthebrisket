...
for r in pa:
    ...
    if (
        dirx != r.get("marketGapDirection")
        or (mag is None) != (r.get("marketGapMagnitude") is None)
        or (mag is not None and abs(mag - r["marketGapMagnitude"]) > 1e-9)
    ):
        bad += 1
        if len(ex) < 5:
            ex.append(
                (
                    ...
                    dirx,
                    mag,
                    r.get("marketGapDirection"),
                    r.get("marketGapMagnitude"),
                )
            )
...
for r in pa:
    for k, m in (r.get("sourceRankMeta") or {}).items():
        if m.get("rawRank") != m.get("effectiveRank"):
            diff[k] += 1
        n += 1
...
for b in sorted(byband):
    print(" ranks", b, "-", b + 99, dict(byband[b]))
