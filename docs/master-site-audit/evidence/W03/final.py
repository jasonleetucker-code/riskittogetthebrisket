...
for r in ranked:
    n = len(r.get("effectiveSourceRanks") or {})
    srs = r.get("sourceRankSpread")
    lg = legacy(n, srs)
    got = r["confidenceBucket"]
    if lg != got:
        diff[(got, lg)] += 1
tot = len(ranked)
s = sum(diff.values())
print(
    f"A) rows where the LEGACY 30/80 rule would give a different bucket: {s}/{tot} = {100*s/tot:.1f}%",
    dict(diff),
)
...
hi = [
    r
    for r in ranked
    if r["confidenceBucket"] == "high" and r.get("hillValueSpread") and r.get("rankDerivedValue")
]
rel = [r["hillValueSpread"] / r["rankDerivedValue"] for r in hi]
over = [r for r in hi if r["hillValueSpread"] / r["rankDerivedValue"] > 0.30]
print(
    f"B) 'high' rows: {len(hi)}; with hillValueSpread/value > 30%: {len(over)} ({100*len(over)/len(hi):.1f}%)"
)
print(
    "   median rel disp by bucket:",
    {
        b: round(
            statistics.median(
                [
                    r["hillValueSpread"] / r["rankDerivedValue"]
                    for r in ranked
                    if r["confidenceBucket"] == b
                    and r.get("hillValueSpread")
                    and r.get("rankDerivedValue")
                ]
            ),
            4,
        )
...
print(
    ...
    [
        (
            r["displayName"],
            r["rankDerivedValue"],
            round(r["hillValueSpread"], 1),
            round(r["hillValueSpread"] / r["rankDerivedValue"], 3),
            len(r.get("effectiveSourceRanks") or {}),
        )
        for r in sorted(over, key=lambda x: -x["hillValueSpread"] / x["rankDerivedValue"])[:5]
    ],
)
...
def flip(rows):
    ...
    for r in rows:
        ...
        if nd != r.get("marketGapDirection"):
            f += 1
    return f, t
