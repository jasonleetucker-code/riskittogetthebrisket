...
def pspread(esr, meta, pools):
    if not esr or len(esr)<2: return None
    pcts=[]
    for k in esr:
        m=meta.get(k) or {}
    ...
    if len(pcts)<2: return None
    if len(pcts)>=5:
        s=sorted(pcts); return s[-2]-s[1]
...
for r in pa:
    ...
    if exp is None and got is None: ok+=1; continue
    if exp is None or got is None: bad+=1; ex.append((r['displayName'],exp,got)); continue
    if abs(round(exp,4)-got)<=0.0002: ok+=1
    else:
        bad+=1
        if len(ex)<10: ex.append((r['displayName'],round(exp,4),got))
