...
for r in pa:
    ...
    for k in list(esr):
        ...
        rv = list(sub.values())
        srs = float(max(rv) - min(rv)) if len(rv) >= 2 else None
        ...
        if len(ex) < 12:
            ex.append(
                (
                    ...
                    best,
                    bestdrop,
                    r.get("sourceRankPercentileSpread"),
                )
            )
print("rows tested (>=3 eff sources, non-pick, ranked):", total)
print(
    "rows where DROPPING ONE SOURCE RAISES confidence:",
    improved,
    round(100 * improved / total, 1),
    "%",
)
for e in ex:
    print("  ", e)
...
for n in sorted(byN):
    ...
    tot = sum(c.values())
    print(
        f'  n={n:2d} tot={tot:4d} high={c["high"]:4d} ({100*c["high"]/tot:5.1f}%) medium={c["medium"]:4d} low={c["low"]:4d}'
    )
