...
|---|---|---|---|
| **3. `percentile_to_value`** (Hill percentile→value) | **NUMERIC — output frozen at 10 ranks, plus all 8 scope constants** | `tests/canonical/test_hill_percentile_constants_tripwire.py` — `_PINNED_OFFENSE_VALUES` at `:87-100` asserted by `:152-160`; `_PINNED` at `:72-84` asserted by `:135` | **blocking** |
...
plus `_PINNED` at `:72-84` (asserted by `test_percentile_constants_are_unchanged` at `:135`), an
equality assertion on **all eight** percentile-form master constants
(GLOBAL, OFFENSE, IDP, ROOKIE × C/S), and
