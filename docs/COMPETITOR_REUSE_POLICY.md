...
> reconciliation (`docs/POST_B_RECONCILIATION_2026-08-14.md`). Body unchanged. Scope boundary, because this policy is easy to over-read:
>
> **This document governs DESIGN-PATTERN reuse, not DATA RIGHTS.** It grants nothing about automated collection
> from any provider. Where it says publicly observable architecture, workflows and interaction patterns are fair
> game, that is compatible with — and does not override — `docs/OWNER_FEATURE_INVENTORY.md` §12 ("not permission
> to clone competitor implementation") and `docs/MASTER_PRODUCT_PLAN.md` §4.9 ("Never copy branding/design
> architecture or use unauthorized/private APIs/content"). Concepts yes; code, branding, copy, private APIs and
> protected assets no. Data collection is authorized only by a per-source permission record.
