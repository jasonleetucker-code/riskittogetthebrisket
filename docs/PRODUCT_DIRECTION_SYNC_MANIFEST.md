...
> reconciliation (`docs/POST_B_RECONCILIATION_2026-08-14.md`) **for its §7 global owner decisions, which are durable**. Body unchanged.
>
> **Its synchronization claims and its acceptance checklist are FALSE AS WRITTEN and are superseded by this
> reconciliation.** Specifically, and verified:
>
> - Its "synchronized through 2026-08-14" stamps do **not** cover the two newest binding owner decisions —
>   #829 Weekly Report Studio and #830 FAAB Market Heat, with owner decisions 47–65 — which existed on `main`
>   and appear nowhere in this branch's authored documents.
> - Its §6 program status and its "staleness detector" name B9a as next authorized. All of B9–B11 are merged.
> - Its §10 acceptance checklist is marked `[x]` for a state that is no longer true.
>
> Treat this file as a **historical drift receipt plus a durable §7 decision list**. It is not a current status
> record. `docs/EXECUTION_PLAN.md` answers what is authorized; `docs/C_SERIES_SCOPE_MANIFEST.md` answers what is
> in scope.
