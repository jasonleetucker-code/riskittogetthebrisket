...
**V1 completion: 75 / 136 = 55.1%.** (72 on `main` post-`#1024`(`V1-94`)/`#1029`(`V1-46`)/`#1021`(`V1-69`/`V1-90`) + `V1-133`/`V1-134`/`V1-135` here, each `NOT STARTED` → `VERIFIED`. Independently re-verified at this reconciliation that #1012's `idpShowCombined` cutover does not touch this evidence: the bridge registry (`config/bridges/bridges_v1.json`) contains only `idpTradeCalc`/`draftSharks`/`dynastyDealer`, `idpShowCombined` has no bridge registry entry and reads its own `is_cross_market` rank through an entirely separate crosswalk in `data_contract.py` — the two mechanisms are disjoint.)
