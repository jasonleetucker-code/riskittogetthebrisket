...
| V1-121 | Release-gate classification: every check has a category | release discipline | L5 | `VERIFIED` | L1 | CI. A real blocker must not be treated as noise, nor a detector block releases. **PROMOTED 2026-08-21; the row was stale.** The 2026-08-16 incident this row exists for — a KTC scrape timeout turning every open PR red and skipping a production deploy — is already repaired: `validate_api_data_contract` partitions every error into exactly one of two lanes, `sourceHealthErrors` (`_is_source_health_error`, matched against `_SOURCE_HEALTH_ERROR_KINDS`) or `structuralErrors`, with no third bucket and no silent drop — `structural_errors = [e for e in errors if not _is_source_health_error(e)]` is exhaustive by construction. Pinned by `tests/api/test_contract_health_lanes.py` (23/23): six known source-health message shapes classify correctly, nine known structural message shapes classify correctly, and `test_the_partition_is_total` asserts `sourceHealthErrors + structuralErrors == errors` on a real synthetic payload — no error can be dropped by a consumer reading the partition instead of the raw list. Reperformed rather than trusted: removed `"pick_count_below_floor:"` from `_SOURCE_HEALTH_ERROR_KINDS` ⇒ RED, the taxonomy test for that exact message misclassifies it as structural; restored ⇒ GREEN, clean tree. |
...
**V1 completion: 71 / 136 = 52.2%.** (68 on `main` post-`#1014`/`#1026` + `V1-69`/`V1-90`/`V1-121` here, each `NOT STARTED` → `VERIFIED`.)
