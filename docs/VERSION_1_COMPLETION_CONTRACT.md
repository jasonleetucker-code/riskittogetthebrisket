...
| V1-32 | Canonical Team Weakness / Need Priority | `C2-WEAK-01` / `C2-U5` | L1 | `IN PROGRESS` | L2 | named V1 scope. ≥5 need definitions to retire. **#914** builds the owner **NOT PROMOTED 2026-08-20, and the L2 evidence is not the reason.** The contract correction is confirmed against this table: V1-32's required level is **L2, not L4**, so a frontend Weakness consumer is NOT a requirement of this row and none should be built to satisfy it. #1007's offline path reproduces on demand and its numbers check out at Integration — `scripts/verify_roster_intelligence.py --offline` gives **216 rungs** (`weakness thresholds scale with teamCount`, PASS) and **215 rung credits** (`weakness credits no player to two rungs`, PASS at EVIDENCE-L2), **0 violations**, 11 passed / 0 failed / 2 unmeasurable, plus `tests/roster_intel/test_verify_offline.py` 5/5. The refusal design is real and worth keeping: the script exits **2** with *"'could not measure' is NOT 'passed'"* and marks the teamAssignment and endpoint-latency checks **UNMEASURABLE** rather than letting offline CPU time masquerade as HTTP latency evidence. **The blocker is the OTHER half of this row's own bar:** it reads *"≥5 need definitions to retire"*, and the backend single-owner discovery guard that would prove the retirement holds is **#1004, which is not on `main`** (checked, not assumed). That is the same shape of gap that got `V1-29` and `V1-88` refused — the repair measured correct while nothing prevented a second owner reappearing — and promoting this row on a strong evidence pack while that half is unguarded would apply a weaker standard than those two were held to. **To close:** land #1004 and reperform the single-owner mutation (a second weakness/need implementation outside the declared owner must fail the guard); the L2 measurement above already stands and does not need redoing.
