...
**V1 completion: 72 / 136 = 52.9%.** (69 on `main` post-`#1014`/`#1026`/`#1024`(`V1-94`) + `V1-133`/`V1-134`/`V1-135` here, each `NOT STARTED` → `VERIFIED`. Independently re-verified at this reconciliation that #1012's `idpShowCombined` cutover does not touch this evidence: the bridge registry (`config/bridges/bridges_v1.json`) contains only `idpTradeCalc`/`draftSharks`/`dynastyDealer`, `idpShowCombined` has no bridge registry entry and reads its own `is_cross_market` rank through an entirely separate crosswalk in `data_contract.py` — the two mechanisms are disjoint. `#1021` — `V1-69`/`V1-90` — is reconciled on its own branch, ahead of this one in the merge order, so the base above already assumes it may land first; whichever of #1021/#1023 actually merges second will need one more reconciliation pass against the other's landed delta.)
