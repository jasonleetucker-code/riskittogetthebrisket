...
| V1-52 | One weekly power-rankings engine | `C5-POW-01` / inv 6.2 | L3 | `IN PROGRESS` | L2 | required current-season-model correctness: 2 competing engines. **Status corrected 2026-08-20 from `NOT STARTED`, which was stale rather than wrong when written** — real V1-52 work has been landing and the ledger had not moved with it. #979 (merge `6c235cb51`) ships contract item **C** — both named lenses of the one canonical engine reachable end-to-end, `GET /api/public/league/rosPower?lens=results_only` over HTTP plus the lens toggle and rank-trend column in `ros-power.jsx` — taking C from SHIPPING_BUT_UNPROVEN to SHIPPING_AND_PROVEN, on top of items **A** (one engine, two lenses) and **B** (refuse to rank rather than fabricate an identifier ordering) already proven by the six commits it isolated off #923. **NOT promoted, and the gap is item D:** one canonical engine consumed everywhere. `src/public_league/power.py`, `power.jsx` and `overview.currentPowerLeader` are byte-unchanged, so two engines still answer "how strong is this team" and the landing card is still wired to the legacy one. The lane declined to redirect it and gave measured reasons rather than deferring vaguely: `rosPower`'s trend is O(all seasons × all weeks) and is excluded from the eager overview walk for that reason, and the headline lens has no `weekRankDelta` computable from data that exists (`team_ros_strength` carries no per-week history), which under MISSING IS NEVER ZERO must stay `None`. #923 stays unmerged and that is still correct — its Step 5 retirement IS item D. Target level remains **L2**, and the outstanding L2 measurement is production-scale 12-owner lens-agreement, which needs the gitignored `data/ros/team_strength/latest.json` |
...
**V1 completion: 76 / 136 = 55.9%.** (73 on `main` post-`#1024`(`V1-94`)/`#1029`(`V1-46`)/`#1021`(`V1-69`/`V1-90`)/`#1033`(`V1-121`) + `V1-133`/`V1-134`/`V1-135` here, each `NOT STARTED` → `VERIFIED`. Independently re-verified at this reconciliation that #1012's `idpShowCombined` cutover does not touch this evidence: the bridge registry (`config/bridges/bridges_v1.json`) contains only `idpTradeCalc`/`draftSharks`/`dynastyDealer`, `idpShowCombined` has no bridge registry entry and reads its own `is_cross_market` rank through an entirely separate crosswalk in `data_contract.py` — the two mechanisms are disjoint.)
