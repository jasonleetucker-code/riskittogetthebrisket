...
| V1-51 | One playoff-probability engine | `C5-PLAY-01` / inv 6.1 | L3 | `IN PROGRESS` | L2 | required current-season-model correctness. **Half of this row is now DONE and measured in production; the other half is a live defect, measured in the same request.** **FIXED — the headline disagreement is gone.** The row was opened on "2 engines disagree 7 vs 6 spots". Measured on the deployed build (`e37d2786e`, deploy completed 03:22 UTC 2026-08-20) via the public league API: `playoffOdds.playoffSpots = 7`, `rosPlayoffOdds.playoffSeeds = 7`, `rosChampionship.playoffSeeds = 7`, `byeSeeds = 1` on both ROS engines, and `playoffStructure` resolves `{playoffTeams: 7, byeTeams: 1, source: "league_settings"}`. All three agree, and they agree with the league's own settings rather than a constant. #919 + #930 (`playoff_field_size = min(playoff_seeds, len(seeded_owners))`, `playoff_seeds` threaded) and #934 delivered that. **NOT VERIFIED — the two engines still answer the SAME state with different vocabulary, and this is no longer a code reading: it is measured live.** In the current preseason state (`weeksPlayed: 0`) the identical question gets three different answers: `playoffOdds` → `numSims: 0`, `simulated: false`, `unsimulable` present; `rosPlayoffOdds` → `n_simulations: 0`; `rosChampionship` → **`championshipOdds: []` with `n_simulations: 10000`**, no `unsimulable` key, no `simulated` flag, `rosStrengthAvailable: true` and a fully resolved bracket. Every signal on that payload says "we had what we needed and ran ten thousand simulations", and the result is empty — indistinguishable to a consumer from "simulated, and nobody has a path". That is MISSING IS NEVER ZERO inverted, live on the deployed build. Cause: `src/ros/championship.py:215`, the `if not distributions:` branch, which is the branch preseason actually takes. Its own sibling at `:193` has the correct shape, and `src/public_league/playoff_odds.py:775-791` does it correctly under a comment stating that it shares vocabulary deliberately because *"two engines must not invent different words for the same state"*. Pre-existing; not introduced by #934, and not widened at the merge desk. **To close:** give `:215` the `unsimulable` shape with `n_simulations: 0` (and a `simulated` flag consistent with its siblings), a test that goes RED without it, then re-measure the three endpoints and record the L2 statement |
