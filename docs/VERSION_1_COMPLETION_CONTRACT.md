...
| V1-51 | One playoff-probability engine | `C5-PLAY-01` / inv 6.1 | L3 | `IN PROGRESS` | L2 | required current-season-model correctness: 2 engines disagree 7 vs 6 spots. #919 / #930 / #934 have all landed and #930's P1 fix is intact on `main` (`src/ros/championship.py:82` `playoff_field_size = min(playoff_seeds, len(seeded_owners))`, `playoff_seeds` threaded — re-verified at Integration on the #934 merge tree). **NOT promoted 2026-08-20 — the two engines still diverge, measured on `main` `e37d2786e`.** `src/public_league/playoff_odds.py:775-791` answers an unsimulable league with `numSims: 0`, `simulated: False` and an explicit `unsimulable` block, under a comment stating that it deliberately shares vocabulary with the ROS engine because *"two engines must not invent different words for the same state"*. `src/ros/championship.py:215` — the `if not distributions:` branch — returns empty `championshipOdds` with **`n_simulations: n_simulations` (10000)**, no `simulated` flag and **no `unsimulable` key**, so a consumer reads "10,000 simulations ran and everyone is at 0%" when nothing was simulated. Its own sibling branch 20 lines above (`:193`) has the correct shape, which is what makes this a divergence rather than an unmade decision. Pre-existing; not introduced by #934, and not widened at the merge desk. **To close:** give `:215` the same `unsimulable` shape (`n_simulations: 0`), a test that goes RED without it, and the L2 measured statement |
...
| V1-63 | Manager-level Sharp concentration | inv 4.6 | L4 | `IMPLEMENTED_UNVERIFIED` | L1 | **NOT promoted 2026-08-20, and the previously-named blocker is NOT the real one.** #927 (merge `df93e5ac1`) fixed the defect this row named — `personManagerQuality` returned `1.0` for zero voters; it now returns `None`, and its two siblings `personAgreement` / `networkConcentration` agree. That repair is **L1-complete and mutation-proven at Integration**, each mutation confirmed APPLIED with the changed line quoted before it was counted: (M1) `consensus.py:118` `else None`→`else 1.0` ⇒ RED `test_zero_voters_reports_unknown_manager_quality_not_maximum`; (M2) `consensus.py:120` undefined concentration `None`→`0.0` ⇒ RED `test_concentration_is_undefined_not_diversified_when_nothing_is_weighted`; (M3) restore `float(raw_quality or 1.0)` ⇒ RED `test_an_explicitly_zero_quality_manager_does_not_vote_at_full_weight`; (M4) `personAgreement` `None`→`1.0` ⇒ RED `test_zero_voters_reports_unknown_manager_quality_not_maximum`; (M5) `market.py:58` caps `0.34`→`1.0` ⇒ RED `test_one_manager_cannot_dominate_an_asset`. 44 pass restored. **Why that still does not verify this row:** `docs/OWNER_FEATURE_INVENTORY.md:132` defines inv 4.6 / `W15-F009` (P1) as *"No per-player manager concentration published"* at `src/sharp/roster_percentage.py:582-625` — the **Sharp Roster Percentage** board, where one manager's five teams render as an 83% sharp roster percentage with nothing on the row saying it is one human. Measured on merged `main` `8b9c28289`: that module contains **no** concentration/distinct-manager field at all (grep for `oncentration|distinctManager|uniqueHuman|managersBehind|perPerson|humanCount` → NONE). `sampleWarning` warns only that the **roster** count is small, which is a different statement; `uniqueManagers` exists only inside the `buySell` join, not on the percentage. The concentration capping that #911 confirmed lives in `src/sharp/market.py` (the Buy/Sell tracker), a different surface from the one the finding cites. **Remaining blocker, precisely:** publish per-player manager concentration on the roster-percentage row so the denominator's rosters-not-people rule is visible where the percentage is read. The lane flagged this ambiguity rather than claiming it ("an Integration reading, not mine to declare") and was right to; the canonical inventory decides it, and it decides against promotion |
...
| V1-95 | Awards do not exist before games are played | `C9-AWARD-01` | L3 | `IMPLEMENTED_UNVERIFIED` | L2 | live production defect: eight awards manufactured with zero games. **#937 merged 2026-08-20 (`5feade0eb`). L1 COMPLETE; L2 UNMEASURABLE here — not promoted.** **L1:** `tests/public_league/test_awards_need_games.py` 9 pass. Two mutations, each confirmed APPLIED with the changed line quoted: (MA1) `awards.py:2327` rename `row["awardsUnavailable"]` so an explicit refusal degrades to an empty list ⇒ RED `test_no_games_is_an_explicit_refusal_not_an_empty_success` — the missing-is-never-zero shape is genuinely pinned, not merely described; (MA2) `awards.py:2310` `if not _has_begun(season):` → `if False:`, removing the gate outright ⇒ **4 named REDs** including `test_a_season_with_zero_games_manufactures_no_awards` and `test_the_named_defect_awards_are_specifically_gone`. **L2 is UNMEASURABLE on the committed data, and that is reported rather than rounded to a pass.** `data/public_league/snapshot.json` holds seasons **2025 (13 awards) and 2024 (9 awards) only — both fully played**; there is no unplayed season, so the case the gate exists for does not arise. Measured both ways on that snapshot: **with the gate and with the gate removed the output is identical** (2 seasons, 13 + 9 awards, `awardsUnavailable: None` throughout), i.e. the gate is provably inert on the data available here. That is `unmeasurable`, not `pass`, in the five-state vocabulary #932 just landed. **To close:** run the same measurement against the production snapshot, where the 2026 season Sleeper flipped to `in_season` before NFL Week 1 actually exists |
