...
|---|---|---|---|
| **Hill master curves** (`HILL_GLOBAL_*`, `HILL_OFFENSE_*`, `HILL_IDP_*`, `HILL_ROOKIE_*` in `src/canonical/player_valuation.py`) | KTC, IDPTC, DynastyNerds, DynastyDaddy CSVs (the 4 value-based sources) | `scheduled-refresh.yml` material-data commit (normally every 2h) → `refit-hill-curves.yml` → raw refit + Hill Autopilot | **AUTO**, event-driven on new market data. Raw refits only create evidence; canonical OFFENSE promotion requires tournament, forward-persistence, scope and board-impact gates. No manual rebaseline pins. |
