...
| **AC-10** | ~~Investigate `KTC_TradeDB` / `KTC_WaiverDB` returning `partial` with *"no playerID→name mapping available"*, `valueCount: 0`~~ **CLOSED 2026-08-18.** Both retired: the identity map they gated on was built inside `if "content" in dir()`, and `content` was bound only inside the failed-scrape branch, so both sub-sources could only run when the primary KTC scrape had failed. Neither had a live consumer. The crowd-FAAB path is unaffected — it is served by `scripts/fetch_crowd_faab.py`, which now resolves 200/200 claims instead of 150/200 | Live degradation in the crowd-FAAB path |
