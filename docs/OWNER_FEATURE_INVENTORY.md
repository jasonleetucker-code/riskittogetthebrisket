...
| 2.9 | **Best Trade to Send Each Team** | For every other team in the league, one best mutually defensible player-only offer | New | **OWNER DECISION 2026-08-14: BUILD.** No dedicated surface exists — zero hits repo-wide. Nearest machinery (the finder's per-opponent iteration, suggestions' fit labels) ranks a global list. Hard rules: players only, equal counts, no picks, canonical WIN for us, opponent EVEN-or-better on KTC or IDP TC with whole-package **native** coverage; missing/unsupported/imputed assets cannot create a fake opponent win; honest no-result; ranked for mutual defensibility, not maximum exploit; recommendation only, never a silent send. Binding spec: `docs/trade/TRADE_GENERATION_PREFERENCES_AND_REFINEMENT_SPEC.md`; backlog §2.3; manifest `C7-BEST-TRADE` | C7 | 2.2, 2.3, 2.4, 2.10, 2.11, 1.1, 1.2, 1.3, 1.5, 7.1 | **M–L** | KEEP — NEW BUILD |
