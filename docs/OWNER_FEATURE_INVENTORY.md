...
| # | Feature | Purpose | New/Existing | Current status at HEAD | Phase | Deps | Scope | Classification |
|---|---|---|---|---|---|---|---|---|
| 10.1 | **Hill curve refit lifecycle** | Fit → tournament → forward-validate → impact-gate → promote → monitor → rollback, fail-closed automatic | Existing | **OWNER UPDATE 2026-09-08:** material market-data refreshes refit the Hill curve; `docs/valuation/HILL_AUTOPILOT_V2.md` authorizes automatic canonical OFFENSE promotion only after the codified persistence, scope, row-health, board-impact, fresh-paired, hard-test and rollback gates clear. Raw refits still cannot write production; GLOBAL/IDP remain evidence-gated and cannot ride OFFENSE evidence. This supersedes ADR-008's Hill-specific human-only promotion boundary without changing the separate adaptive-weighting prohibition below. | B1 | 7.2 | — | KEEP — INFRASTRUCTURE/FOUNDATION |
