...
| 7.13 | **24/7 adaptive data-freshness orchestration** | Keep every decision-relevant external input as fresh as legitimately/reliably possible without one monolithic scrape or abusive polling | New canonical orchestration over existing jobs | **OWNER DECISION 2026-09-24 (#1423 / T-NEW-21).** Today the repo has a monolithic 2-hour Scheduled Data Refresh plus several source-specific jobs and strong content-freshness diagnostics, but no single per-source adaptive scheduler/SLA owner. Required end state: independent staggered collectors, adaptive/event-relative cadence, provider budgets/backoff/circuit breakers, isolation, LKG/fail-honest semantics, immutable point-in-time observations, next-due/SLA diagnostics, alerts on missed freshness, and high-volatility treatment for future odds/market feeds. Reuse the existing freshness clocks/weighting rather than fork them. | C4 | C4-SRC-* | **L** | KEEP — NEW FOUNDATION / CONSOLIDATE ORCHESTRATION |
