...
>
> **§19.2 is the ONLY third-party data-use permission record anywhere in this repository**, which is why this
> document was promoted regardless of PR #809's eventual disposition. Read it with its own §19.2 caveat in force:
> it records that *the owner reports having received* direct permission from KeepTradeCut. The underlying grant
> artifact — evidence, contact, granted scope, permitted collection method, rate expectations, attribution,
> redistribution limits and revocation terms — **is not in the repository**, and §19.2 itself requires capturing
> it before production collection. Manifest row `F-EXT-01` tracks that gap as an OWNER DECISION.
>
> **The permission covers KTC *data ingestion*. It is not an authorization for anything else**, and in particular
> it says nothing about submitting packages to KTC for grading — see `docs/CE_REGISTRY.md` and manifest row
> `C7-BEST-TRADE`. **No equivalent record exists for IDP Trade Calculator.**
