...
| Package generation | `src/packages/` (**created ~2026-08-20**, self-declared "Canonical package-construction substrate (V1-36/C3-PKG-01)"; deliberately NOT under `src/trade/`) | **CORRECTED 2026-08-20 (C10 audit): owner cell previously said `src/trade/`, which the substrate's own docstring explicitly rejects — documentation drift, not a code defect.** Measured: 3 of 4 generators migrated (`src/trade/finder.py`, `src/trade/angle.py`, `src/roster_intel/packages.py`); `src/trade/suggestions.py` (the largest, 85 KB) has not | `C3-PKG-01` |
| Value Adjustment | `src/trade/ktc_va.py` (to become sole Python port) | **RE-VERIFIED 2026-08-20 (C10 audit): monkeypatch, rounding divergence and dead V12/V13 exports are FIXED (CLAUDE.md, confirmed by direct read + a green parity test) — 2 Python ports remain (`ktc_va.py` + `src/public_league/trade_grading.py`, both parity-tested against each other and against `frontend/lib/trade-logic.js`), not the 5 this row named** | `C3-VA-01` |
| Equalizers (post-VA gap balancer) | *(to be created)* one owner, called from `C3-PKG-01` | **ADDED 2026-08-20 (C10 audit) — this row had no owner-map entry at all, itself a gap.** `findBalancers` exists twice with divergent rules: Python `src/trade/suggestions.py:1403` (fires at gap 256, returns up to 2, direction-aware; called `:1822`) vs JS `frontend/lib/trade-logic.js:1264` (fires at 350, returns up to 5, one-sided band) — acknowledged by name in `src/trade/ktc_va.py:41-42`. Neither is retired | `C3-EQ-01` |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C3-PKG-01` | ONE shared package generator | `src/packages/` (**corrected 2026-08-20** — was `*(new)* src/trade/`, which `src/packages/__init__.py`'s own docstring rejects) | DUPLICATED — 3 of 4 generators migrated (`finder.py:19`, `angle.py:97-100,609,629`, `roster_intel/packages.py:368,379`); `src/trade/suggestions.py` (85 KB) has not | One engine; every surface a consumer | CONSOLIDATE | `C2-SIM-01` | audit both | P2 | trade | — | zero-diff on finder + suggestions |
| `C3-VA-01` | ONE Value Adjustment per runtime, with parity proof | `src/trade/ktc_va.py` + `frontend/lib/trade-logic.js` | **RE-VERIFIED 2026-08-20 (C10 audit, `docs/master-site-audit/evidence/W30/dead-code-map.csv` D-032):** monkeypatch deleted, dead V12/V13 exports removed, rounding divergence fixed — all per CLAUDE.md's trade-engine section, confirmed here by direct code read (`src/trade/market_value_adjustment.py` is now a 0-computation re-export of `ktc_va.py`) + a green parity run (`tests/public_league/test_trade_grade_parity.py`, 8 tests/50 subtests). **2 Python implementations remain**, not 5: `ktc_va.py` (canonical) and `src/public_league/trade_grading.py` (a deliberate, own-docstring-justified server-side port for public grading, parity-tested against both `ktc_va.py`'s fixture and the JS port) — same sanctioned-duplication shape as `D-039`. Whether that second Python port should still collapse to satisfy this row's "one Python port" target is a Lane 2 (`C3`, trade substrate) call, not resolved by this audit | One Python port, one JS port, parity-tested; monkeypatch deleted; dead V12/V13 exports removed | CONSOLIDATE | — | audit both, W30-F005 | P2 | trade | — | parity fixture ±0 |
