...
|---|---|---|---|
| Player identity | `src/identity/unified_mapper.py` + `src/utils/name_clean.py` | **3 independent matchers** | `C1-ID-01` |
...
| Canonical pick value | same pipeline (discount → tether) | HOLDS for 2026–2029; completeness gate open | `C1-PICK-01` |
...
| Historical value / as-of evidence | *(to be created)* immutable temporal ledger | **4 fragmented stores, 1 with no history at all** | `C1-HIST-01` |
| Lineup / slot assignment | `src/ros/lineup.py::solve_optimal_assignment` | **exact owner exists; 6 competing greedy fills, 2 in production** | `C2-LINE-01` |
| Replacement level / PAR | *(to be consolidated)* | **5 implementations** | `C2-REPL-01` |
| Team Strength | `src/ros/team_strength.py` (to become sole service) | **4 competing notions** | `C2-STR-01` |
...
| Roster simulation (exact before/after) | *(to be created)* on the exact lineup solver | **delta-only; `team_impact` reimplements lineup** | `C2-SIM-01` |
| Package generation | *(to be created)* one shared engine under `src/trade/` | **4 independent generators** | `C3-PKG-01` |
| Value Adjustment | `src/trade/ktc_va.py` (to become sole Python port) | **5 implementations; 1 installed by import-time monkeypatch; rounding divergence** | `C3-VA-01` |
| Whole-package market coverage | `src/league_intel/cross_market.py` | **correct and DISCONNECTED — zero production importers** | `C3-XMKT-01` |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C1-RET-01` | KTC crowd-FAAB rolling window is durably retained | `scripts/fetch_crowd_faab.py` | PARTIAL — accumulator + timer exist; output gitignored, unbacked-up, absent from any durable path | Retained + backed up | REPAIR | — | audit 2 §19 | P3 | ret | `RET` | artifact on prod, in backup |
| `C1-RET-02` | Canonical board history is provably recording | `src/snapshots/board_store.py` | PROOF-REQUIRED — the recorder's own docstring: "every day it is not running is a day of evidence that cannot be recovered later", and whether it runs is unobservable off-host | Observable + backed up | PRODUCTION-PROOF | — | audit 2 §19 | P3 | ret | `RET` | the read-only `Scoring Snapshot Diagnostics` workflow reports whether the recorder is scheduled — the only way to answer this from outside the prod host |
| `C1-RET-03` | `rank_history.jsonl` stall is detectable | `src/api/rank_history.py` | PARTIAL — best-effort append, fresh-scrape-gated, single copy, silent on failure | Monitored + backed up | REPAIR | — | audit 2 §19 | P3 | ret | `RET` | watchdog alert |
| `C1-RET-04` | Scoring card at a date | `league_registry.write_scoring_snapshot` | **ABSENT BY CONSTRUCTION** — `tmp.replace(path)` overwrites; yesterday's card is destroyed | Append-only history keyed by (league, observed_at) | IMPLEMENT | `C1-HIST-01` | audit 2 §19 | P3 | ret | `RET` | history rows exist |
| `C1-RET-05` | Sleeper trending adds are recorded | `src/adapters/sleeper_trending.py` | **ABSENT** — 15-minute TTL, never persisted; the waiver-heat series has never existed | Persisted time series | IMPLEMENT | `C1-HIST-01` | audit 2 §19 | P3 | ret | `RET` | series exists |
| `C1-RET-06` | Own-league trade events are captured before the window drops them | `src/api/sleeper_overlay.py` | PARTIAL — live 365-day rolling fetch, depth-2 chain, **no `transaction_id` emitted** | Durable event ledger with a stable key | IMPLEMENT | `C1-ACQ-01` | audit 2 §19 | P3 | ret | `RET` | ledger rows |
| `C1-RET-07` | Per-source raw ingest + identity reports resume | scraper / `scripts/identity_resolve.py` | **HALTED 2026-04-20** — 116 days unrecorded; `/api/scaffold/identity` serves the 04-20 file as current | Collection resumed or the surface honestly labelled | REPAIR | — | audit 2 §19 | P3 | ret | `RET` | fresh artifacts |
| `C1-RET-08` | `playerctx` history actually lands | `scripts/refresh_playerctx.py` | **0 snapshots ever committed** despite a push mechanism and a weekly timer | Snapshots landing | REPAIR | — | audit 2 §19 | P3 | ret | `RET` | committed snapshots |
| `C1-ID-01` | One player-identity owner | `src/identity/unified_mapper.py` | DUPLICATED — 3 independent matchers | Scraper and data-contract matching become adapters | CONSOLIDATE | — | inventory 7.x | P2 | id | — | parity harness |
| `C1-ID-02` | One pick identity, end to end | *(new)* asset-identity contract | DUPLICATED — 7 representations; collapse is documented in 3 places; original owner discarded | One canonical pick id carrying ownership and lineage | IMPLEMENT | `C1-ID-01` | ledger 90/91, TC-20/21 | P2 | id | — | no collapse in the trade surface |
| `C1-PICK-01` | Every valid pick through 2029 has a finite canonical value | `_compute_unified_rankings` | PARTIAL — 2029 **is** priced but synthetically (a verbatim clone of 2028 tiers × 0.53); 2030+ absent; slot picks exist for 2026 only | Automated census passes: value exists, finite, not zero-as-missing, identity stable, provenance stamped, and rankings == trade == API == export == ownership == mobile == desktop | IMPLEMENT | `C1-ID-02` | contract §10, T-NEW-15 | P6 | pick | — | census green |
| `C1-PICK-02` | Generic ↔ exact-slot transition without a second asset | *(new)* | ABSENT | Same identity survives the transition | IMPLEMENT | `C1-ID-02` | TC-20/21/22 | P2 | pick | — | transition test |
...
| `C1-HIST-01` | One immutable as-of value/provenance ledger | *(new)* | DUPLICATED — 4 fragmented stores | One contract; fidelity labels `exact` / `nearest-prior` / `reconstructed` / `partial` / `unavailable` | IMPLEMENT | `C1-ID-01` | replay spec §6, aging spec | P2 | hist | — | replay determinism test |
| `C1-HIST-02` | Historical pick values are first-class | `C1-HIST-01` | **STRUCTURALLY ABSENT for every pick actually traded** — all 90 of 2026's pick rows are excluded from `rank_history` because they carry no rank | Pick values recorded per date | IMPLEMENT | `C1-HIST-01`, `C1-ID-02` | aging spec | P2 | hist | — | rows exist |
| `C1-HIST-03` | Board-history `rankChange` is deterministic | `data_contract._stamp_rank_changes` | PARTIAL — reads then unconditionally overwrites a single-slot cache, so build N+1 diffs against build N's own output; keyed by bare `canonicalName` | Derived from the dated log, or written only on fresh-scrape promotion; collision-keyed | REPAIR | `C1-HIST-01` | B residual (a) | P2 | hist | — | back-to-back build determinism test |
| `C1-ACQ-01` | Acquisition history / cost basis | *(new)* | ABSENT — zero fields, zero tables | Per-league transaction/event ledger with lineage | IMPLEMENT | `C1-ID-01`, `C1-HIST-01` | inventory 7.11, ledger 77–88 | P2 | hist | — | ledger rows |
| `C1-ACQ-02` | Historical roster reconstruction | `C1-ACQ-01` | ABSENT — the code says so: "Historical rosters aren't archived" | Roster at a date, with fidelity labels | IMPLEMENT | `C1-ACQ-01` | replay spec §10 | P2 | hist | — | reconstruction test |
| `C1-ACQ-03` | Pick lineage / trade trees (CE-18) | `C1-ACQ-01` | ABSENT — `originalOwnerId` is one hop, not a chain; the intel ledger strips origin | Full chain | IMPLEMENT | `C1-ID-02`, `C1-ACQ-01` | CE-18 | P2 | hist | — | chain test |
| `C1-CONF-01` | Confidence naming defects renamed before new consumers exist | `src/api/confidence.py` | PARTIAL — 24 priced rows labelled `"none" / "None — unranked"`; `identityConfidence` means id-resolution; `marketConfidence` means dispersion | Accurate names; aliases for existing consumers; **no methodology change** | REPAIR | — | B residuals (b)(c)(d) | P2 | id | — | rename test + consumer census |
| `C1-SRC-01` | Multi-format dynasty source archive | `docs/MULTI_FORMAT_SOURCE_NORMALIZATION_SPEC.md` | ABSENT | Source-native 1QB/SF/TEP/IDP variants preserved; KTC's four calibration states counted as ONE opinion | IMPLEMENT | `C1-HIST-01` | #809 | P3 | src | — | archive + registry |
| `C1-SRC-02` | Game type proven per feed, fails closed on `UNKNOWN` | `_RANKING_SOURCES` | COMPLETE for current sources | Explicit per-endpoint proof | COMPLETE-ALREADY | — | `CLAUDE.md` | P2 | src | — | existing tests |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C2-LINE-01` | One lineup/slot assignment | `src/ros/lineup.py::solve_optimal_assignment` | DUPLICATED — exact owner exists; **6 competing implementations, 2 of them serving production** (`starter-slots.js` on /terminal and /rosters, `team_impact.py` on `/api/trade/simulate`) | Every consumer calls the exact solver | CONSOLIDATE | — | audit both | P2 | roster | — | zero-diff harness across surfaces |
...
| `C2-STR-01` | Canonical Team Strength | `src/ros/team_strength.py` | DUPLICATED — 4 competing notions; `/terminal`'s is a raw value sum that excludes picks | One service; position-count thresholds and Top-N slots per spec; **must agree with the canonical lineup solve** | IMPLEMENT | `C2-LINE-01`, `C2-REPL-01` | inventory 1.1, MPP §4.1 | P2 | roster | — | cross-surface parity |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C3-PKG-01` | ONE shared package generator | *(new)* `src/trade/` | DUPLICATED — 4 independent generators (`finder`, `suggestions`, `angle`, `roster_intel/packages`); the only architecturally-correct one is disconnected | One engine; every surface a consumer | CONSOLIDATE | `C2-SIM-01` | audit both | P2 | trade | — | zero-diff on finder + suggestions |
| `C3-VA-01` | ONE Value Adjustment per runtime, with parity proof | `src/trade/ktc_va.py` + `frontend/lib/trade-logic.js` | DUPLICATED — 5 implementations; one installed into the finder by an **import-time monkeypatch**; Python banker's rounding vs JS `Math.round` divergence | One Python port, one JS port, parity-tested; monkeypatch deleted; dead V12/V13 exports removed | CONSOLIDATE | — | audit both, W30-F005 | P2 | trade | — | parity fixture ±0 |
...
| `C3-XMKT-01` | Whole-package native market coverage | `src/league_intel/cross_market.py` | **DISCONNECTED** — correct, measured (n=476 paired players), zero production importers; `angle.py` is the intended call site and is not rewired | Wired as the canonical owner; every package evaluated entirely within one market that covers every asset | CONSOLIDATE | `C3-PKG-01` | WS-J F-1 | P2 | trade | — | `angle.py` rewired |
...
| `C3-TOPO-01` | Generated-trade topology constraint | `C3-PKG-01` | ABSENT | `abs(players_A − players_B) <= 1`; picks excluded from the count. 1v1, 2v1, 1v2, 3v2, 2v3 allowed; 3v1, 1v3, 4v2, 2v4 disallowed. **Supersedes the exact-equal-player-count rule** | IMPLEMENT | `C3-PKG-01` | #841/#842 supersession | P2 | trade | — | topology test |
...
| `C4-MTL-02` | KTC Trade Database ingestion lane | `C4-MTL-01` | ABSENT; ingestion currently skipped for missing id→name mapping | Ingested within the granted permission scope, KTC-labelled provenance | IMPLEMENT | `C4-MTL-01`, `F-EXT-01` | spec §19.2 | P3 | mkt | `EXT` `OD-01` | grant artifact captured |
...
| `C4-KTC-01` | KTC TradeDB/WaiverDB identity mapping repair | `Dynasty Scraper.py` | PARTIAL — a `content` binding defect yields unjoinable `Player#<id>` names; the accumulator script is the live mitigation | Joinable names | REPAIR | `C1-ID-01` | W05-F005 | P3 | mkt | — | join rate |
...
1. **Native coverage of every asset in the package** on the chosen market — `src/league_intel/cross_market.py`
   already implements exactly this, with measured evidence, and is disconnected (`C3-XMKT-01`).
2. **Faithful application of that market's package algorithm** — `C3-VA-01`, once the five implementations are
   collapsed to one.
...
| Explicitly superseded owner rules | 5 (2028/2029 unpriced posture · player-MVP eligibility gate · `unified_signal_engine` ownership claim · Best Trade `no draft picks` · Best Trade exact-equal-player-count) |
