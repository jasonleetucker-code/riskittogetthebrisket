...
| `C1-ID-02` | One pick identity, end to end | `src/identity/picks.py` | **DELIVERED 2026-08-16 (checkpoint pending)** — canonical owner created; census measured 97 records / 39 deduplicated definition sites (not 7; see `docs/identity/C1_ID_02_CENSUS.md`); six RED classes reproduced on real data and closed; consumers adapted byte-inert (board 0/1093); `assetId` stamped on pickDetails; intel-ledger re-key deferred to `C1-ACQ-01/03` with the collision documented at the owner | One canonical pick id carrying ownership and lineage | IMPLEMENT | `C1-ID-01` | ledger 90/91, TC-20/21 | P2 | id | — | no collapse in the trade surface |
