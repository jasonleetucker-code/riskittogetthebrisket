...
| Package generation | `src/packages/construction.py` (**CORRECTED 2026-08-20** — shared mechanics substrate: `PackageShape`, `topology_is_allowed`, `enumerate_packages`/`enumerate_sides`, `package_key`) | **3 of 4 generators already consume it** — `finder.py` and `angle.py` fully, `roster_intel/packages.py` only for identity (no capacity/constraint integration yet). **CORRECTED 2026-08-24 — `suggestions.py` is no longer a holdout**: `#1079` (`a3e7bdb82`) migrated it onto the owner's identity primitives (`PackageAsset`, `adapt_assets`, `side_key`), measured byte-identical over 176 real suggestions. It still hand-rolls its own fixed 1v1/2v1 shapes, which was a deliberate preservation requirement, so the MECHANICS half is unmigrated by design; `C3-PKG-01` stays PARTIAL for that reason and because the identity guard was measured not to enforce single ownership — see `V1-36` in the V1 contract | `C3-PKG-01` |
...
| **P6 — model / methodology** | anything that changes a published number | build-time budget stated and measured | n/a | n/a | n/a | backtest + calibration + pinned-input provenance | **champion/challenger; automatic promotion only through an owner-approved, deterministic, fail-closed policy with rollback** | every promotion — human or automatic — records the evidence and decision; Hill Autopilot v2 (PR #1315) is the current approved exception to the former human-only rule, confirmed directly with the owner 2026-09-09 after an independent safety review flagged the removed human-approval step against this row's own boundary — see `docs/OWNER_FEATURE_INVENTORY.md` §10.1 |
...
| `C1-PICK-02` | Canonical pick lifecycle: generic → exact-slot → retired-active-state without a second asset | `market_resolution` (C1-U3) + `pick_value_resolution` (C1-U6) + canonical active-asset lifecycle | **Lifecycle implemented 2026-09-25 by #1442 (`bfdb238d8`); production pick-horizon check passed 2026-09-26.** **PARTIAL after owner extension 2026-09-24 (#1414 / T-NEW-20).** The original generic→exact identity transition is CLOSED (PR #871, merge `ce8a8341a`): one `LeaguePickIdentity` resolves finite at every basis with no second asset minted. New required lifecycle step is not yet proven: once the league rookie draft is complete **and** drafted rookies are rostered, that class becomes RETIRED from current selectors while the same historical identity/snapshots/trade references remain resolvable. 2026 is the immediate stale-active case. | Same identity survives generic→exact and then leaves the ACTIVE current-asset set after proven draft completion + rookie rostering; retirement never deletes history or means value zero; unknown state is conservative | IMPLEMENT | `C1-ID-02` | TC-19/20/21/22; #1414 / T-NEW-20 | P2 | pick | — | existing transition test + post-draft retirement/unknown-state/history-resolution tests |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C3-PKG-01` | ONE shared package generator | `src/packages/construction.py` (**CORRECTED 2026-08-20** — a real shared mechanics substrate: `PackageShape`, `topology_is_allowed`, `enumerate_packages`/`enumerate_sides`, `package_key`) | PARTIAL, not the prior "4 independent generators" — `finder.py` and `angle.py` fully consume the substrate; `roster_intel/packages.py` (disconnected — no frontend caller) consumes only the identity helper, keeping a duplicate `_check_legality` (**the `constraints.py` half was closed by `#962`**; the `roster_capacity.py` half remains open, so "zero integration" is corrected to partial); `suggestions.py` (the live `/api/trade/suggestions` endpoint) **is NO LONGER the holdout as of `#1079` (`a3e7bdb82`, 2026-08-24)** — it consumes the owner's identity primitives, proven byte-identical over 176 real suggestions across 12 rosters × 3 pool regimes. It deliberately retains its hand-rolled fixed 1v1/2v1 shapes carrying already-tuned product logic (an explicit preservation requirement, not an omission), so the MECHANICS half is unmigrated by design. Still PARTIAL, and the blocking reason is now the guard rather than the code: the identity guard was measured **not** to enforce single ownership — deleting the owner import and reimplementing both helpers locally leaves 11/11 guard tests and 1608 relevant tests green. See `V1-36` in `docs/VERSION_1_COMPLETION_CONTRACT.md` for the full audit and the exact remaining acceptance | One engine; every surface a consumer | CONSOLIDATE | `C2-SIM-01` | audit both | P2 | trade | — | zero-diff on finder + suggestions |
...
| `C3-CALC-01` | Trade Calculator maturity | `frontend/app/trade/` + shared package/asset representation | PARTIAL — 2–5 sides, KTC import, unpriced honesty all work; Analyze Trade, real-trade evidence, comparable trades, historical fidelity absent. **Owner clarification 2026-09-24 (#1415 / T-NEW-02): repeatable/generic quantity and duplicate-label identity safety remain required and unproven end to end.** | TC-01…TC-30 end-state gate **plus** T-NEW-02 quantity semantics: generic/repeatable assets may occur more than once or quantity >1; multiple distinct real owned picks may coexist even when they render the same tier/label; the exact same unique owned pick cannot be accidentally double-counted; math/remove-one/share/persistence/export/mobile-desktop preserve quantity and identity | IMPLEMENT | `C3-PKG-01`, `C4-MTL-01`, `C1-ID-02` | `docs/TRADE_CALCULATOR_MARKET_EVIDENCE_EXPANSION_SPEC.md`; #1415 / T-NEW-02 | P1 | trade | — | TC-30 gate + quantity/identity round-trip acceptance examples from #1415 |
...
| id | capability | owner | status | final | disposition | deps | source | prof | lane | flag | evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `C8-PSI-01` | Design primitives, tokens, shell, breakpoints, focus states | `frontend/components/ds/` | PARTIAL — DS, PSI editorial tokens, shell, bundle gates and axe E2E exist; gallery/docs drift and per-route proof remain (`docs/ui/UI_PARALLEL_LEDGER.md`, 2026-09-24) | No-regret primitives landed; token direction resolved | IMPLEMENT | `C0-PSI-01` | `docs/PREMIUM_SPORTS_INTELLIGENCE_DESIGN_NORTH_STAR.md` | P1 | psi | `OD-05` | token contract |
| `C8-PSI-02` | Reference route migration (Rankings first) | `frontend/app/rankings/` | PARTIAL — Rankings and Player File PSI code exists; current full proof/legacy retirement not certified (`docs/ui/UI_PARALLEL_LEDGER.md`) | One route migrated behind a rollback boundary | MIGRATE | `C8-PSI-01`, `C0-PERF-01` | north star §5 | P1 | psi | — | per-route parity + perf |
| `C8-PSI-03` | Route-by-route migration | frontend | PARTIAL — several PSI-scoped routes exist, including merged Trade #1294; remaining parity/evidence in `docs/ui/UI_PARALLEL_LEDGER.md` | Every route; `/league`, sharp/market and admin/tools are still on legacy | MIGRATE | `C8-PSI-02` | north star | P1 | psi | — | per-route proof |
...
| `C8-PERF-03` | Rankings board windowing | `frontend/app/rankings/` | SHIPPED — windowing landed and `VERIFIED` as `V1-106` (`a9136e13e`). The former "59.5 FPS" figure is retired: it came from #760's first harness, which paced its own scroll with `setTimeout(16)` and whose own header retracts the number as a self-measurement artifact (corrected 2026-08-25, Integration) | Windowed | UX/PERF | `C0-PERF-01` | `V1-106` | P1 | perf | — | FPS harness |
...
| `C8-A11Y-01` | Accessibility instrumentation | frontend | PARTIAL — structural ratchet and axe E2E exist; populated reference-route coverage and current production proof remain | Automated a11y checks per route | IMPLEMENT | `C8-PSI-01` | audit 2 §21 | P1 | psi | — | axe in CI |
