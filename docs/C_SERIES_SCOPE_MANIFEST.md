...
| `C4-SRC-04` | 24/7 adaptive staggered freshness orchestration | canonical source scheduler/registry over existing collectors + source-health/freshness primitives | ABSENT as one canonical orchestration owner — current baseline is a 2-hour monolithic refresh plus ad-hoc/source-specific jobs | Every active external source has an independent target/adaptive cadence, provider budget/terms policy, next-due state, isolation, stagger/jitter, event acceleration where warranted, backoff/circuit breaker, LKG/fail-honest promotion, immutable point-in-time evidence, SLA diagnostics and missed-freshness alerts. API/webhook/stream first where authorized. Future odds feeds retain timestamped line/price history and use event-relative quota-aware cadence. Reuse existing fetch/any-change/broad-change/upstream clocks; no second freshness model. | IMPLEMENT | `C4-SRC-02`, `C1-HIST-01` | #1423 / T-NEW-21 (owner 2026-09-24) | P3 | src | — | source registry complete; independent staggered runs; rate-budget tests; hung-source isolation; SLA/next-due observability; alert proof; interactive responsiveness under load; rollback |
