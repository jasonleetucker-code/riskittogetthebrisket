...
| `C1-PICK-04` | Active draft-pick lifecycle / post-draft retirement | canonical pick identity/lifecycle owner + active-asset availability | ABSENT as one canonical cross-surface rule; 2026 picks remain current-site assets after the 2026 rookies are already rostered | A draft class is ACTIVE until the league rookie draft is complete **and** drafted rookies are rostered; then it is RETIRED from present-tense selectors/decision surfaces without deleting historical identity, snapshots or trade references. Missing/unknown state never retires early and retirement never means value zero. One lifecycle predicate, no page-local year filters. | IMPLEMENT | `C1-ID-02`, `C1-PICK-01` | #1414 / T-NEW-20 (owner 2026-09-24) | P2 | pick | — | 2026 absent from active selectors; future synthetic rollover; history still resolves; unknown state conservative |
...
| `C3-CALC-04` | Repeatable/generic asset quantities + duplicate-label identity safety | `C3-PKG-01` + Trade Calculator state/serialization | PARTIAL/UNPROVEN — existing T-NEW-02 requires generic pick quantities, but no completion evidence proves the full UI→serialization→math round-trip or distinct-owned-pick same-label case | Generic/hypothetical/repeatable assets may occur multiple times / quantity >1; distinct real owned picks must coexist even when display labels/tier match; the exact same unique owned-pick identity cannot be accidentally duplicated. Value, VA, package math, equalizers, remove-one, share/persistence/export and mobile/desktop preserve all copies/identities. | IMPLEMENT | `C3-PKG-01`, `C1-ID-02` | #1415 / T-NEW-02 clarification (owner 2026-09-24) | P2 | trade | — | two generic Mid 2027 1sts count twice; two distinct same-label owned picks coexist; one unique pick cannot double-count; round-trip parity |
