...
68. **FLEX is a separate supplement, per T-NEW-19.** Superflex is folded into QB demand *before* the 1.5×, and is never treated as ordinary RB/WR/TE FLEX; regular offensive and IDP FLEX each create `ceil(1.5 × real flex slots)` after the dedicated cores, filled from the highest-valued remaining legally eligible players; each player is counted at most once.
