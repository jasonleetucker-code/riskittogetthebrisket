...
|---|---|---|---|---|---|
| `C0-R` | [#875](https://github.com/jasonleetucker-code/riskittogetthebrisket/pull/875) | `ad95bd6` | **RED on the inherited `main` defect below, and only that** — run 31987737878: `test_derived_2029_value_uses_measured_year_step` (ratio 0.9857), plus four advisory-lane source-coverage rails on the same nine rows. **Re-measured on `main@85b13689` (2026-08-17 08:3xZ): 8 of 18 tier cells still violate**, down from 9 — and the drop is noise, not progress: the structural cells (Mid 4th/5th/6th) are unchanged at ratio 1.035 while `Mid 2nd` flipped only because its 2028 twin drifted a point. Only a scrape that re-runs the pick anchors can clear it. Its own diff is documentation and standing-invariant tests | every later unit — this is the record that authorizes them | pending |
...
| `C2-U1` | [#880](https://github.com/jasonleetucker-code/riskittogetthebrisket/pull/880) (stacked on #879) | `a0261cd9` | **GREEN** — run 32005935926 SUCCESS on the exact head, all 24 steps (07:43:58Z). Two earlier heads failed and both were MINE, not inherited: `1fa38ade` on the coercion gate (four baseline allowances went stale because retiring the greedy engines deleted the coercions they allowed) and `16965978a` on the hard gate (`test_baseline_is_internally_consistent` — my prune scripts left `count: 687` beside 682 entries). Board measured inert **0/1111 values, 0 ranks**; confidence measured inert **0/1111 rows**. The canonical solver was verified against **Sleeper's own awarded lineups, 10/10**, before being trusted — the two production greedies it replaced score 0/10 and 5/10 | `C2-U2` · `C2-U3` · `C2-U4` · `C2-U6` · `C2-U9` · `C5-PLAY-01` — the root of the C2 phase | pending — checklist in `docs/lineup/C2_U1_CANONICAL_LINEUP.md` §10 |
