...
| S-2 | Map scraper-run names ↔ registry keys so a disposition round-trips and a run-level "complete" decomposes into which boards arrived | **OPEN — now measured, and it has a consequence.** `KTC_TEP` is a sub-product held in `FULL_DATA`, not a member of `active_sites`, so `sites_meta` never emits `ktcSfTep` and it is absent from `raw.sites` entirely. **Correction (2026-08-18):** this item previously concluded that the scrape-promotion gate `server.py::_missing_expected_sites` therefore *cannot* watch it. That was wrong — the function reads `siteStats` as well as `sites` (server.py:1179-1182), and `siteStats` carries `ktcSfTep` with a real count (644). `expectedSites.offense` is now `["ktcSfTep"]`, measured inert across all 176 archives (identical 4 blocked either way). Audit finding **F-10** is this defect seen from the consequence end: losing the entire TE++ board moved 444 of 468 offense rows while every gate said healthy. F-10 closed the contract floor and the scraper site_raw floor; the `raw.sites` half is still open and is the actual S-2 work. **The `playerCount: null` design proposed for this row is REFUTED (2026-08-18) and must not be implemented — a present key defeats `dict.get`'s default, so `None > 0` raises `TypeError` inside the handler that marks a scrape FAILED, breaking the two-hourly refresh. `failedSources` is also ABSENT from all 176 archives, so a detector keyed on it observes nothing. Full record and the required posture: audit F-12.** |
...
| S-3 | Health vocabulary must distinguish vendor-unchanged / stale / fetch-failed / parser-failed / credential / blocked / retired / future / archive-only | **OPEN — one state added 2026-08-18, seven still missing.** Audit **F-11** added `unmeasurable` (registered, but neither a `_last_success` stamp nor a CSV — UNKNOWN, not fresh) to `scripts/watchdog_freshness.py` and `scripts/check_source_health.py`, because the previous behaviour was to drop such a source from the population entirely and report "22 fresh / 0 hard-stale", exit 0. `vendor-unchanged` already exists as content staleness. Still unrepresented on any surface: parser-failed, credential, blocked, retired, future, archive-only. Audit **F-18** additionally closed the divergence between the two freshness OWNERS for the seven sources where the contract was laxer than the alert engine (22 of 22 budgets disagreed; seven were 168-720h against a fetch-success stamp measuring 1.1-1.7h). Which of the two owners is CANONICAL is still open under this item. Neither F-11 nor F-18 invents an age, threshold or staleness verdict — F-18 applies the decision the repo already made on PR #532 — so **S-6 remains untouched and owner-gated** |
