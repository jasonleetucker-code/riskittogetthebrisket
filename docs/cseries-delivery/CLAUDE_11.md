...
## 14. TEMPORARY MODE — V1 70% support: read-only adversarial audit of V1-49 and V1-52

Per an explicit owner instruction received after §13: all POST-V1 work (#966,
#973, #969, #981, and any further projection/WAR/Game Day expansion) is
**frozen** for this turn — no simulator, no projection-source expansion, no
edits to any of the four open PRs. This session's only job was a **read-only**
adversarial audit of two V1 rows Claude 3 owns, delivered as PR comments and a
`docs/WORK_CLAIMS.md` entry, never as file edits. No files under Claude 3's
ownership were touched.

**V1-49 (individual special-teams scoring)** — DISAGREE. #915's SAF-alias and
blocked-kick repairs are sound and well-tested (28/28), and the SAF fix is
correctly structural (`_IDP_POSITIONS` now derives from `POSITION_ALIASES`
rather than a second table). One residual `Coverage.GAP` found, same defect
shape: `kick_ret_td`/`punt_ret_td`/`idp_def_pr_td`/`idp_def_kr_td` are real
Sleeper scoring keys with no scoring path in `src/nfl_data/realized_points.py`
— `sleeper_stat_line_from_row` only ever emits the combined `st_td`, never a
KR/PR split, though `pbp_weekly.py` already reads `return_touchdown` (so this
is reconstructable, not a genuine data gap). Not consumer-reachable on
`dynasty_main` today; reachable on any league that configures the split rule.
Invisible to `test_individual_special_teams.py` because its live-card fixture
never exercises the split. Delivered as a comment on merged PR #915 (no open
PR exists for this row).

**V1-52 (one weekly power engine)** — DISAGREE that consolidation is complete
on `main` today; AGREE the ledger is honest about it. Only #979 is merged;
#996 is open/`dirty`; #1009 is open, based on #996's own unmerged branch.
Verified against live `main`, not PR descriptions: two engines still coexist
and can show different "power leader #1" answers on the same page
(`public_contract.py:74,352` still routes the overview widget through legacy
`power.py`; `LeagueClient.jsx` still mounts both components behind a toggle
defaulting `true`). More seriously: `src/ros/power_v2.py`'s PPG accumulator
walks every season with no per-season reset — the exact cross-season
contamination defect `docs/CANONICAL_WEEKLY_POWER_RANKINGS_SPEC.md` was
written to repair — while `recentAvg`/`all_play` in the same engine ARE
correctly season-scoped. Playoff Odds preservation, trend semantics, and the
global unrankable case all check out (with one precision correction: #979
did edit `playoff_sim.py`, safely, removing genuinely dead code — not
"untouched" as could be read from the PR descriptions alone). A real,
untested per-owner unrankable gap was found (`games == 0` for one owner falls
through to a silent `ppg = 0.0`, bypassing the renormalization that protects
the global case) — reachability from live Sleeper data not verified.
Real-league L2 measurement doesn't exist yet (only synthetic timing
evidence), matching the row's own honest `IN PROGRESS` status. Full six-item
findings delivered as PR comments on #996 and #1009.

Both findings also logged as dedicated rows in `docs/WORK_CLAIMS.md`, marked
`AUDIT FINDING (not claimed, not my work)` and status `done` (the audit
itself is complete; the underlying gaps remain Claude 3's to close), so a
future session in any lane can see them without re-reading three PR bodies.

Deliberately not done this turn, per the freeze: nothing on #966/#973/#969/#981
was touched, rebased, or re-examined; no simulator, ensemble aggregation, or
capture script was written.
